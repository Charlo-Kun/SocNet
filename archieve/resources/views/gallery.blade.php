...
<div class="container">
        ...
        <div class="left">
        <h2 class="fas fa-image">Gallery</h2>
        </div>
  </div>

    @if (Auth::user()->posts->count() > 0)
    <table class="posts-table">
        <div class="gallery-container"> @foreach(Auth::user()->posts()->latest()->get() as $post)
            @if ($post->post_picture)
            <div class="gallery-item"><img class="gallery-picture" src="{{ asset('storage/' . $post->post_picture) }}" alt="post Picture" onclick="showImage('{{ asset('storage/' . $post->post_picture) }}')">
 </a></div>
...
<script src="{{ asset('path/to/jquery.magnific-popup.min.js') }}"></script>
</body>
